...
std::string default_screenshot_filepath() {
  ...
  stream << "screenshots/screenshot-"
         << std::put_time(&localtime, "%Y-%m-%d-%H-%M-%S") << ".png";
  return stream.str();
}
...
void GLAPIENTRY opengl_callback(GLenum source, GLenum type, GLuint id,
  ...
  std::cout << "OpenGL Debug Message " << id << " (type: " << _type << ") of "
            << _severity << " raised from " << _source << ": " << message
            << std::endl;
}
...
// This is the main class function that run the whole application (Initialize,
// Game loop, House cleaning). run_for_frames decides how many frames should be
// run before the application automatically closes. if run_for_frames == 0, the
// application runs indefinitely till manually closed.
int our::Application::run(int run_for_frames) {
  ...
  auto win_config = getWindowConfiguration(); // Returns the WindowConfiguration
                                              // current struct instance.

  // Create a window with the given "WindowConfiguration" attributes.
  // If it should be fullscreen, monitor should point to one of the monitors
  // (e.g. primary monitor), otherwise it should be null
  GLFWmonitor *monitor =
      win_config.isFullscreen ? glfwGetPrimaryMonitor() : nullptr;
  // The last parameter "share" can be used to share the resources (OpenGL
  // objects) between multiple windows.
  window = glfwCreateWindow(win_config.size.x, win_config.size.y,
                            win_config.title.c_str(), monitor, nullptr);
  if (!window) {
  ...
  // This will make sure that OpenGL and the main thread are synchronized such
  // that message callback is called as soon as the command causing it is
  // called. This is useful for debugging but slows down the code execution.
  ...
  // This part of the code extracts the list of requested screenshots and puts
  // them into a priority queue
  using ScreenshotRequest = std::pair<int, std::string>;
  std::priority_queue<ScreenshotRequest, std::vector<ScreenshotRequest>,
                      std::greater<ScreenshotRequest>>
      requested_screenshots;
  if (auto &screenshots = app_config["screenshots"]; screenshots.is_object()) {
    auto base_path =
        std::filesystem::path(screenshots.value("directory", "screenshots"));
  ...
  // Call onInitialize if the scene needs to do some custom initialization (such
  // as file loading, object creation, etc).
  if (currentState)
    currentState->onInitialize();

  // The time at which the last frame started. But there was no frames yet, so
  // we'll just pick the current time.
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    // Render the ImGui commands we called (this doesn't actually draw to the
    // screen yet.
    ImGui::Render();

    // Just in case ImGui changed the OpenGL viewport (the portion of the window
    // to which we render the geometry), we set it back to cover the whole
    // window
    auto frame_buffer_size = getFrameBufferSize();
    glViewport(0, 0, frame_buffer_size.x, frame_buffer_size.y);

    // Get the current time (the time at which we are starting the current
    // frame).
    double current_frame_time = glfwGetTime();

    // Call onDraw, in which we will draw the current frame, and send to it the
    // time difference between the last and current frame
    if (currentState)
      currentState->onDraw(current_frame_time - last_frame_time);
    last_frame_time =
        current_frame_time; // Then update the last frame start time (this frame
                            // is now the last frame)

#if defined(ENABLE_OPENGL_DEBUG_MESSAGES)
    // Since ImGui causes many messages to be thrown, we are temporarily
    // disabling the debug messages till we render the ImGui
    glDisable(GL_DEBUG_OUTPUT);
    glDisable(GL_DEBUG_OUTPUT_SYNCHRONOUS);
#endif
    ImGui_ImplOpenGL3_RenderDrawData(
        ImGui::GetDrawData()); // Render the ImGui to the framebuffer
#if defined(ENABLE_OPENGL_DEBUG_MESSAGES)
    ...
    while (requested_screenshots.size()) {
      if (const auto &request = requested_screenshots.top();
          request.first == current_frame) {
        if (our::screenshot_png(request.second)) {
          std::cout << "Screenshot saved to: " << request.second << std::endl;
        } else {
          std::cerr << "Failed to save a screenshot to: " << request.second
                    << std::endl;
        }
    ...
    // If a scene change was requested, apply it
    while (nextState) {
      // If a scene was already running, destroy it (not delete since we can go
      // back to it later)
      if (currentState)
...
// Sets-up the window callback functions from GLFW to our (Mouse/Keyboard)
// classes.
void our::Application::setupCallbacks() {
  ...
  // It is replaced by an inline function -lambda expression- as it is not
  // needed to create a seperate function for it. In the inline function we
  // retrieve the window instance and use it to set our (Mouse/Keyboard) classes
  // values.
  ...
  glfwSetCursorPosCallback(window, [](GLFWwindow *window, double x_position,
                                      double y_position) {
    auto *app = static_cast<Application *>(glfwGetWindowUserPointer(window));
    if (app) {
  ...
  glfwSetScrollCallback(window, [](GLFWwindow *window, double x_offset,
                                   double y_offset) {
    auto *app = static_cast<Application *>(glfwGetWindowUserPointer(window));
    if (app) {
