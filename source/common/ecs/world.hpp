...
public:
  ...
  void markForRemoval(Entity *entity) {
    // TODO: (Req 8) If the entity is in this world, add it to the
  ...
  void deleteMarkedEntities() {
    // TODO: (Req 8) Remove and delete all the entities that have been marked
    // for removal
    for (auto itr = markedForRemoval.begin(); itr != markedForRemoval.end();
         itr++) {
      entities.erase(*itr);
      delete *itr;
    }
  ...
  void clear() {
    // TODO: (Req 8) Delete all the entites and make sure that the containers
    // are empty
