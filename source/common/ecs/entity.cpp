...
glm::mat4 Entity::getLocalToWorldMatrix() const {
  ...
  glm::mat4 m = localTransform.toMat4();  // Start with the local transform of the current entity

  // Traverse the parent chain and accumulate the transforms
  Entity *ptr = parent;
  while (ptr) {
  ...
  return m; // return the final result matrix
}
