...
void ForwardRenderer::initialize(glm::ivec2 windowSize,
  ...
  if (config.contains("sky")) {
    ...
    // Load the sky texture (note that we don't need mipmaps since we want to
    // avoid any unnecessary blurring while rendering the sky)
...
void ForwardRenderer::render(World *world) {
  ...
  std::sort(
      ...
      [cameraForward](const RenderCommand &first, const RenderCommand &second) {
        ...
        //  drawn before "second".
        glm::vec3 v1 = (first.center - cameraForward);
        glm::vec3 v2 = (second.center - cameraForward);
        double l1 = pow(v1.x, 2) + pow(v1.y, 2) + pow(v1.z, 2);
        double l2 = pow(v2.x, 2) + pow(v2.y, 2) + pow(v2.z, 2);
        return (l1 > l2);
      });
  ...
  glm::mat4 P =
      camera->getProjectionMatrix(glm::ivec2(windowSize.x, windowSize.y));
  glm::mat4 VP = P * V;
  ...
  for (auto x : opaqueCommands) {
    our::Material *material = x.material;
    our::ShaderProgram *shader = material->shader;
    material->setup();
    shader->set("transform", (VP * x.localToWorld));
    x.mesh->draw();
  }
...
if (this->skyMaterial) {
  ...
  glm::mat4 V = camera->getViewMatrix();
  glm::mat4 P = camera->getProjectionMatrix(glm::ivec2(windowSize.x, windowSize.y));
  glm::mat4 VP = P * V;
  glm::mat4 cameraMatrix = glm::inverse(VP);
  glm::vec3 cameraPos = glm::vec3(cameraMatrix[3]);
 

  // TODO: (Req 10) Create a model matrix for the sky such that it always
  glm::mat4 skyModelMatrix = glm::translate(glm::mat4(1.0f), cameraPos);

  // TODO: (Req 10) We want the sky to be drawn behind everything (in NDC
  ...
  skyMaterial->shader->set("transform", alwaysBehindTransform * skyModelMatrix);
