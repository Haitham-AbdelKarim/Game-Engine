...
public:
  ...
  void update(World *world, float deltaTime) {
    ...
    // If the left mouse button is pressed, we get the change in the mouse
    // location and use it to update the camera rotation
    if (app->getMouse().isPressed(GLFW_MOUSE_BUTTON_1)) {
      glm::vec2 delta = app->getMouse().getMouseDelta();
      rotation.x -=
          delta.y *
          controller->rotationSensitivity; // The y-axis controls the pitch
      rotation.y -=
          delta.x *
          controller->rotationSensitivity; // The x-axis controls the yaw
    }

    // We prevent the pitch from exceeding a certain angle from the XZ plane to
    // prevent gimbal locks
    if (rotation.x < -glm::half_pi<float>() * 0.99f)
      rotation.x = -glm::half_pi<float>() * 0.99f;
    if (rotation.x > glm::half_pi<float>() * 0.99f)
      rotation.x = glm::half_pi<float>() * 0.99f;
    // This is not necessary, but whenever the rotation goes outside the 0 to
    // 2*PI range, we wrap it back inside. This could prevent floating point
    // error if the player rotates in single direction for an extremely long
    // time.
    ...
    fov = glm::clamp(
        fov, glm::pi<float>() * 0.01f,
        glm::pi<float>() *
            0.99f); // We keep the fov in the range 0.01*PI to 0.99*PI
    camera->fovY = fov;

    // We get the camera model matrix (relative to its parent) to compute the
    // front, up and right directions
    ...
    // If the LEFT SHIFT key is pressed, we multiply the position sensitivity by
    // the speed up factor
