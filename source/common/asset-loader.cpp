...
//    { shader_name : { "vs" : "path/to/vertex-shader", "fs" :
...
//      The key is the parameter name, e.g. "MAG_FILTER", "MIN_FILTER",
//      "WRAP_S", "WRAP_T" or "MAX_ANISOTROPY" The value is the parameter value,
//      e.g. "GL_NEAREST", "GL_REPEAT"
//  For "MAX_ANISOTROPY", the value must be a float with a value >= 1.0f
template <> void AssetLoader<Sampler>::deserialize(const nlohmann::json &data) {
  if (data.is_object()) {
    for (auto &[name, desc] : data.items()) {
...
template <> void AssetLoader<Mesh>::deserialize(const nlohmann::json &data) {
  if (data.is_object()) {
    for (auto &[name, desc] : data.items()) {
      std::string path = desc.get<std::string>();
...
//              the type will decide which class will be instanced in the
//              function "createMaterialFromType" found in "material.hpp"
//      "shader" where the value must be the name of a loaded shader
//      "pipelineState" (optional) where the value is a json object that can be
//      read by "PipelineState::deserialize" "transparent" (optional,
//      default=false) where the value is a boolean indicating whether the
//      material is transparent or not
//      ... more keys/values can be added depending on the material type (e.g.
//      "texture", "sampler", "tint")
template <>
...
template <>
void AssetLoader<AudioLibrary>::deserialize(const nlohmann::json &data) {
  if (data.is_object()) {
    for (auto &[name, desc] : data.items()) {
...
void deserializeAllAssets(const nlohmann::json &assetData) {
  ...
  if (assetData.contains("audios"))
    AssetLoader<AudioLibrary>::deserialize(assetData["audios"]);
}
