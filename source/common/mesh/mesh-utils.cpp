...
our::Mesh *our::mesh_utils::loadOBJ(const std::string &filename) {
  ...
  // Since the OBJ can have duplicated vertices, we make them unique using this
  // map The key is the vertex, the value is its index in the vector "vertices".
  ...
  if (!tinyobj::LoadObj(&attrib, &shapes, &materials, &warn, &err,
    ...
    std::cerr << "Failed to load obj file \"" << filename
              << "\" due to error: " << err << std::endl;
    return nullptr;
  }
  if (!warn.empty()) {
    std::cout << "WARN while loading obj file \"" << filename << "\": " << warn
              << std::endl;
  }

  // An obj file can have multiple shapes where each shape can have its own
  // material Ideally, we would load each shape into a separate mesh or store
  // the start and end of it in the element buffer to be able to draw each shape
  // separately But we ignored this fact since we don't plan to use multiple
...
// Segments define the number of divisions on the both the latitude and the
// longitude
