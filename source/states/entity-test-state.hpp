...
// This is a helper function that will search for a component and will return
// the first one found
template <typename T> T *find(our::World *world) {
...
class EntityTestState : public our::State {
  ...
  void onDraw(double deltaTime) override {
    ...
    // First, we look for a camera and if none was found, we return (there is
    ...
    for (auto &entity : world.getEntities()) {
      ...
      // skip this entity)
      our::MeshRendererComponent *meshRenderer =
          entity->getComponent<our::MeshRendererComponent>();
      if (meshRenderer == nullptr)
        continue;
      // TODO: (Req 8) Complete the loop body to draw the current entity
      //  Then we setup the material, send the transform matrix to the shader
      //  then draw the mesh
